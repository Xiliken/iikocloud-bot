...
async def check_orders():
    ...
    for user in users_db:
        last_order = iikoapi.get_last_order(user_phone=user[0].phone_number)
        ...
        if last_order and last_order["whenClosed"]:
            last_order_datetime = datetime.datetime.strptime(last_order["whenClosed"], "%Y-%m-%d %H:%M:%S.%f").replace(
